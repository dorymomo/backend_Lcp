...
public class LoginOkController implements Execute{
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (loginUser != null) {
			...
			session.setAttribute("organNo", loginUser.getOrganNo());
			session.setAttribute("organName", loginUser.getOrganName());
			
			session.setMaxInactiveInterval(3 * 60 * 60); 
			
			System.out.println(loginUser.getOrganNo());
			System.out.println(loginUser.getOrganName());
